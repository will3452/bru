...
@section('main-content')
    <h1 class="h3 mb-4 text-gray-800">{{ __('List of Album') }}</h1>
    <div class="d-flex justify-content-between">
        <a href="{{ route('books.index') }}" class="btn btn-primary btn-sm mb-2"><i class="fa fa-angle-left"></i> Back</a>
        <a href="{{ route('albums.create') }}" class="btn btn-primary btn-sm mb-2"><i class="fa fa-plus"></i> Add New Album</a>
    </div>
    <div class="d-flex mt-4 flex-wrap">
        @foreach ($albums as $book)
        <div  class="m-4 text-center " style="position:relative;" x-data="{showInfo:false}" x-on:mouseover="showInfo = true" x-on:mouseout="showInfo = false">
            ...
            <div class="mt-2">
                <a href="{{ route('albums.show', $book) }}">{{ $book->title }}</a>
            </div>
