...
class ApiPurchaseController extends Controller
{
    ...
    public function purchaseWork(Request $request)
    {
        ...
        if ($data['work_type'] == 'art') {
            ...
            if ($status) {
                $user->logs()->create([
                    'content' => 'Got myself a new Art Scene',
                ]);
            }
            ...
            if ($status) {
                $user->logs()->create([
                    'content' => 'Got myself a new Audio Book',
                ]);
            }
        }
        if ($data['work_type'] == 'podcast') {
            ...
            if ($status) {
                $user->logs()->create([
                    'content' => 'Got myself a new Podcast',
                ]);
            }
